...
secrets are required: evaluation uses the placeholder defaults from
`modules/common/settings.nix`.

```bash
nix fmt                                   # format all .nix files
nix flake check --no-build --all-systems  # evaluate both hosts
```
...
Deploy with a `path:` flake reference (`nixos-rebuild switch --flake path:.#server …`)
so the untracked `local.nix` is included. While any setting still holds a placeholder
(a `CHANGE_ME…` string or the example domain), `lanbat.placeholderSettings` is non-empty
and switching is refused. Check it with
`nix eval path:.#nixosConfigurations.<host>.config.lanbat.placeholderSettings`.
Give new settings that must be filled in a `CHANGE_ME_…` default so the check covers them.
